...
export class Quest11Component implements OnInit {
  ...
  onDrop(event: CdkDragDrop<string []>){
    ...
    checkAnswers(): number{
        ...
        for (let i = 0; i < 3; i++) { // CHECK BOX 1

        if(this.box1[i] == this.box1results[0]){
          counter++;
        }
        else if (this.box1[i] == this.box1results[1]){
          counter++;
        }
        else if (this.box1[i] == this.box1results[2]){
          counter++;
        }
      }

      for (let i = 0; i < 3; i++) { // CHECK BOX 2
      ...
      result = (10*(((100/12)*counter)/100)).toFixed(1);
